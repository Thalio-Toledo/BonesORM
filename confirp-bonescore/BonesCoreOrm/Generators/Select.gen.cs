...
public class SelectGen(Config config) : IGenerator
{
    ...
    private string GetForeignKey(PropertyInfo daughterEntity, Type parentEntity) =>
        parentEntity
            .GetProperties()
            .FirstOrDefault(prop => prop.GetCustomAttribute<ForeignkeyOf>()?.EntityName == daughterEntity.Name)?.Name
                ??
    ...
    private string GetForeignKeyFromSubSelect(Type nestedListClassType, Type MainTableType) =>
        nestedListClassType
            .GetProperties()
            .FirstOrDefault(prop => prop.GetCustomAttribute<ForeignkeyOf>()?.EntityName == MainTableType.Name)?.Name ??
