...
using static BonesCore.Models.GroupByModel.Config;

namespace BonesCore.Models;
...
public class JoinModel
{
    ...
    public void SetScript()
    {
        ...
        {
            if (this.ParticularScript is not null)
            {
                this.Script = $"{this.Type} JOIN {Table} (NOLOCK) {this.Alias} ON {this.ParticularScript}";
            }
            else
            {
                this.Script = $"{this.Type} JOIN {Table} (NOLOCK) {this.Alias} ON {this.Alias}.{this.PrimaryKey} = {this.LeftAlias}.{ForeignKey}";
            }

            this.Script += this.AdditionalScript;
        }
    }
}
...
public class GroupByModel
{
    ...
    public class ColumnSet
    {
        ...
        public string GetValue()
        {
            ...
            {
                return $"'{this.ValueString.Replace("'", "''")}'";
            }
            ...
            {
                return $"{(this.ValueBool == true ? '1' : '0')}";
            }
            if (this.ValueDate != DateTime.MinValue)
            {
                return $"'{this.ValueDate:yyyyMMdd HH:mm:ss.fff}'";
            }
